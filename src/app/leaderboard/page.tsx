...
export default async function LeaderboardPage() {
  ...
  const { data: recentBattles } = await supabase
    .from('battles')
    .select('id, player1_username, player2_username, winner, created_at, round_number')
    .eq('status', 'finished')
      ...
      <div className="max-w-4xl mx-auto space-y-12">
        ...
        <section>
          <header className="mb-6">
            <h2 className="text-2xl md:text-3xl font-black italic text-slate-200 tracking-tight">
              RECENT BATTLES
            </h2>
          </header>
          ...
          <div className="bg-slate-900/50 backdrop-blur-md rounded-2xl border border-slate-800 overflow-hidden shadow-2xl">
            <ul className="divide-y divide-slate-800/50">
              {recentBattles?.map((b) => (
                <li key={b.id} className="p-4 md:p-6 flex items-center justify-between hover:bg-slate-800/30 transition">
                  <div className="flex items-center gap-4">
                     <span className={`text-lg font-bold ${b.winner === 'p1' ? 'text-amber-400 drop-shadow-md' : 'text-slate-400'}`}>
                        {b.winner === 'p1' && '👑 '}@{b.player1_username}
                     </span>
                     <span className="text-slate-600 text-sm font-black italic">VS</span>
                     <span className={`text-lg font-bold ${b.winner === 'p2' ? 'text-amber-400 drop-shadow-md' : 'text-slate-400'}`}>
                        {b.winner === 'p2' && '👑 '}@{b.player2_username}
                     </span>
                     {b.winner === 'draw' && <span className="ml-2 text-xs font-bold text-slate-400 bg-slate-800 px-2 py-1 rounded">DRAW</span>}
                  </div>
                  <div className="text-right">
                     <div className="text-slate-300 font-mono font-bold">{b.round_number - 1} Rounds</div>
                     <div className="text-xs text-slate-500 mt-1">{new Date(b.created_at).toLocaleDateString()} {new Date(b.created_at).toLocaleTimeString()}</div>
                  </div>
                </li>
              ))}
              {(!recentBattles || recentBattles.length === 0) && (
                <div className="p-8 text-center text-slate-500 font-bold italic">No recent battles found.</div>
              )}
            </ul>
          </div>
        </section>
