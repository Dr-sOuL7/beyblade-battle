...
export function getBattleKeyboard(battleId: string) {
  return {
    inline_keyboard: [
      [
        { text: 'Attack ⚔️', callback_data: `action:${battleId}:attack` },
        { text: 'Defend 🛡️', callback_data: `action:${battleId}:defend` }
      ],
      [
        { text: 'Evade 💨', callback_data: `action:${battleId}:evade` },
        { text: 'Special ✨', callback_data: `action:${battleId}:special` }
      ]
    ]
  };
}

export function getJoinKeyboard(battleId: string) {
  return {
    inline_keyboard: [
      [
        { text: 'Join Fight ⚔️', callback_data: `join_battle:${battleId}` }
      ]
    ]
  };
}
